...
TODE68 的 Profile Landlock 只允许当前沙箱进程读取自己的 `/proc/self`。CoreCLR 必须保持这个 PID，因此每次 terminal 调用只执行一个 officecli 命令，并使用：

```bash
exec /opt/data/bin/officecli <参数>
```

不要直接运行裸 `officecli`，不要把多个 officecli 命令放进同一个 shell 调用，也不要在前后串接其他命令。本文其余代码块中的 `officecli ...` 仅表示参数形式；实际执行时逐条拆分，并替换为上面的 `exec /opt/data/bin/officecli ...`。Profile 已注入 `DOTNET_SYSTEM_GLOBALIZATION_INVARIANT=1`，不要自行安装 ICU 或放宽 `/proc`、Landlock 白名单。
