...
metadata:
  hermes:
    tags: [RAGFlow, Dataset, Retrieval]
platforms: [linux]
prerequisites:
  commands: [python3]
  env_vars: [RAGFLOW_API_URL, RAGFLOW_API_KEY]
required_environment_variables:
  - name: RAGFLOW_API_URL
    prompt: RAGFlow API base URL
  - name: RAGFLOW_API_KEY
    prompt: RAGFlow API key
---
...
```bash
python3 {baseDir}/scripts/datasets.py list --json
python3 {baseDir}/scripts/datasets.py info DATASET_ID --json
python3 {baseDir}/scripts/datasets.py create "Example Dataset" --description "Quarterly reports" --json
python3 {baseDir}/scripts/update_dataset.py DATASET_ID --name "Updated Dataset" --json
python3 {baseDir}/scripts/upload.py DATASET_ID /path/to/file.pdf --json
python3 {baseDir}/scripts/upload.py list DATASET_ID --json
python3 {baseDir}/scripts/update_document.py DATASET_ID DOC_ID --name "Updated Document" --json
python3 {baseDir}/scripts/parse.py DATASET_ID DOC_ID1 [DOC_ID2 ...] --json
python3 {baseDir}/scripts/stop_parse_documents.py DATASET_ID DOC_ID1 [DOC_ID2 ...] --json
python3 {baseDir}/scripts/parse_status.py DATASET_ID --json
python3 {baseDir}/scripts/search.py "query" --json
python3 {baseDir}/scripts/search.py "query" DATASET_ID --json
python3 {baseDir}/scripts/search.py --dataset-ids DATASET_ID1,DATASET_ID2 --doc-ids DOC_ID1,DOC_ID2 "query" --json
python3 {baseDir}/scripts/search.py --retrieval-test --kb-id DATASET_ID "query" --json
python3 {baseDir}/scripts/list_models.py --json
```
