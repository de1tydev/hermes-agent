...
required_environment_variables:
  - name: DASHSCOPE_API_KEY
    prompt: Alibaba Cloud DashScope API key
metadata:
  hermes:
    tags: [Image, Qwen, DashScope]
---
...
```bash
python3 {baseDir}/scripts/generate_image.py --prompt "一副典雅庄重的对联悬挂于厅堂之中" --size "1664*928"
```

Generate and save locally:
```bash
python3 {baseDir}/scripts/generate_image.py --prompt "一副典雅庄重的对联悬挂于厅堂之中" --size "1664*928" --filename "qwen-output.png"
```

With custom model:
Support `qwen-image-max-2025-12-30` `qwen-image-plus-2026-01-09` `qwen-image-plus`
```bash
python3 {baseDir}/scripts/generate_image.py --prompt "a beautiful sunset over mountains" --model qwen-image-plus-2026-01-09
```

## API Key
You can obtain the API key and run the image generation command in the following order.

- Read `DASHSCOPE_API_KEY` from the current Hermes Profile environment.
- Obtain a key from https://dashscope.console.aliyun.com/ when one has not been provisioned.
