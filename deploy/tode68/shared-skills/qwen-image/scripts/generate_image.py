...
Usage:
    python3 generate_image.py --prompt "your image description" --filename "output.png" [--model qwen-image-max|qwen-image-turbo] [--size 1664*928|1024*1024|720*1280|1280*720]
"""
...
def get_api_key() -> str | None:
    """Read the API key only from the Profile-scoped environment."""
...
def main():
    ...
    if not api_key:
        ...
        print("Set DASHSCOPE_API_KEY in the current Hermes Profile.", file=sys.stderr)
        sys.exit(1)
    ...
    try:
        ...
        if args.filename:
            ...
            print(f"IMAGE_PATH: {full_path}")
        else:
            # Return the temporary image URL for the caller to deliver.
