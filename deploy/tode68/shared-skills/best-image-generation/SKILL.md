...
prerequisites:
  commands: [python3]
  env_vars: [EVOLINK_API_KEY]
required_environment_variables:
  - name: EVOLINK_API_KEY
    prompt: EvoLink API key
metadata:
  hermes:
    tags: [Image, EvoLink, Generation]
---
...
Return the absolute output path, then send the file with the current platform's media-delivery tool.
...
- Do not rely on a `MEDIA:` marker in Feishu; send the generated file with the current platform's media-delivery tool.
