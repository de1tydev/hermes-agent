...
class TestAdapterBehavior(unittest.TestCase):
    ...
    def test_reply_thread_opt_out_allows_top_level_fallback(self):
        ...
        class _MessageAPI:
            def reply(self, request):
                calls.append(("reply", request.message_id))
                return SimpleNamespace(success=lambda: False, code=230011)

            def create(self, request):
                calls.append(("create", request.request_body.receive_id))
                return SimpleNamespace(
                    success=lambda: True,
                    data=SimpleNamespace(message_id="om_top_level"),
                )
        ...
        with patch(
            ...
            response = asyncio.run(
                adapter._feishu_send_with_retry(
                    chat_id="oc_chat",
                    msg_type="text",
                    payload=json.dumps({"text": "status update"}),
                    reply_to="om_trigger",
                    metadata={"thread_id": "omt-thread"},
                )
            )
        ...
        self.assertEqual(calls, [("reply", "om_trigger"), ("create", "oc_chat")])
