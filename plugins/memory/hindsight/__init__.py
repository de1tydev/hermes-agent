    ...
    def _do_recall(self, query: str) -> _RecallResult:
        ...
        if self._auto_recall_mental_models:
            try:
                ...
                logger.debug("Mental model recall failed: %s", exc, exc_info=True)
        try:
            if self._prefetch_method == "reflect":
                ...
                count = mental_model_count + num_results
        except Exception as exc:
        ...
        def _do_retain() -> None:
            ...
            try:
                ...
                if retain_async_flag:
                    self._track_retain_ops(response, bank_id)
                succeeded = True
